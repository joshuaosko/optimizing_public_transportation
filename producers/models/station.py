...
class Station(Producer):
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        logger.info(f"station_id: {type(self.station_id)}\ntrain_id:{type(train.train_id)}\ndirection:{type(direction)}\nline:{type(self.color.name)}\nprev_station_id:{type(prev_station_id)}\nprev_direction:{type(prev_direction)}")
        logger.info(f"station_id: {self.station_id}\ntrain_id:{train.train_id}\ndirection:{direction}\nline:{self.color.name}\nprev_station_id:{prev_station_id}\nprev_direction:{prev_direction}")
        try:
